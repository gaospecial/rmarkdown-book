...
在本书前面的章节里 \@ref(intro-widgets)，我们已经简单介绍了 HTML组件 \index{HTML widgets}。**htmlwidgets** 包提供了一个将 R 与 Javascript 绑定的框架。HTML组件可以：
...
- 在R终端中像常规的R绘图一样用于数据分析；

- 植入 R Markdown 文档中；^[注意：仅输出格式为 HTML 时才有交互效果。当输出不是 HTML 时，仅可以展示静态快照。参见 \@ref(intro-widgets)。]

- 可以编入 Shiny 网页应用；

- 保存为独立页面时，可通过 Email 和 Dropbox 分享。
...
目前已经有很多 R 包基于 HTML 组件框架开发，方便 R 用户使用R语言和数据创建 Javascript 应用。对此，本书难以做到面面俱到。用户需要阅读相应包的文档来了解具体用法。本章的内容主要面向意欲在R语言中支持更多的 Javascript 库的开发者，需要对 Javascript 有一定的了解。

## Overview 概述 {#htmlwidgets-overview}
...
只需要做一小点转换工作，就可以用很少的代码创建 HTML 小组件。所有的组件都包括以下几个部分：

1. **依赖**：包括组件需要用到的 Javascript 和 CSS 配置文件（如 Javascript 库文件）。

2. **R绑定**：这是终端用户使用的 R 函数，用来向组件提供输入数据、配置参数，是组件可以被渲染出来。除此之外，还可能包括小组件用在 Shiny APP 时所必须的简短样本函数。

3. **Javascript绑定**：用来将所有材料拼接在一起的 Javascript \index{JavaScript}，将 R 提供的数据和参数传递到 Javascript 库中。
...
HTML 组件总是在 R 包中存在，必须包括所有依赖的源代码。只有这样才能保证 R 在渲染组件的时候可以重复（不需要网络连接和连接服务器）。
...
## A widget example 一个组件的示例 (sigma.js) {#htmlwidgets-sigma}
...
这个函数接受数据、参数设置，并将所有信息传递给 `htmlwidgets::createWidget()` 函数。
...
我们还希望能够在 Shiny 应用程序中使用，因此添加了 Shiny 输出和渲染的样板文件。
